...
- **Automatic dark mode for `<ecl-editor>`** (`ecl-editor`) ([#112](https://github.com/aehrc/ecl-lsp/pull/112)): `theme` previously took a fixed Monaco theme name and nothing else, so an editor embedded in a dark page stayed light unless the host tracked the OS colour scheme itself and rewrote the attribute. `theme="auto"` now resolves `prefers-color-scheme` and follows it live — no reload and no attribute rewrite. `light-theme` and `dark-theme` point `auto` at custom Monaco themes, a new `ecl-theme-change` event reports `{ theme, dark }` so surrounding UI can stay in sync, and the applied theme is readable as a `resolvedTheme` property. The default remains `vs`, so existing embedders are unaffected and `auto` is opt-in. The media query listener is attached only in auto mode and released on disconnect or when switching to a fixed theme.

  Note that Monaco applies themes globally, so all editors on a page share one theme; mixing a light and a dark `<ecl-editor>` is a Monaco limitation rather than a component one.

- **Configurable gutter for `<ecl-editor>`** (`ecl-editor`) ([#112](https://github.com/aehrc/ecl-lsp/pull/112)): `line-numbers="false"` hid the numbers but left Monaco reserving the line-number column, the glyph margin and the folding strip, so the left margin stayed visible and there was no way to reclaim it. A `gutter` attribute now selects `full` (the default), `minimal` (no line numbers, glyph margin kept so the quick-fix lightbulb still renders) or `none` (nothing shown, and the reserved widths zeroed). The individual `line-numbers`, `glyph-margin` and `folding` attributes override whichever preset is in effect, and `line-numbers` additionally accepts Monaco's `relative` and `interval` modes alongside the original booleans. All of these apply at runtime.

### Fixed

- **Element chrome rendered light against `hc-black`** (`ecl-editor`) ([#112](https://github.com/aehrc/ecl-lsp/pull/112)): theme darkness was inferred with `theme.includes('dark')`, which is false for `hc-black`, so the shortcut hints bar and resize handle were drawn in light colours against a black editor. Built-in themes are now looked up explicitly, with the name heuristic kept only for custom themes.
- **`minimap` and `line-numbers` attribute changes were ignored after initialisation** (`ecl-editor`) ([#112](https://github.com/aehrc/ecl-lsp/pull/112)): both fell through to the language-service configuration branch of `attributeChangedCallback` and never reached the editor. All presentation attributes now apply live.
- **Duplicate `vite` installation** ([#110](https://github.com/aehrc/ecl-lsp/pull/110)): the four browser packages each declare `vite ^8.2.2`, but npm was also hoisting a transitive `vite 6.4.3` to the repository root and satisfying the direct dependencies from nested copies. Storybook's builder resolves vite from the root, so the end-to-end suites were exercising the stories on vite 6 while the packages built and shipped on vite 8. Every declared consumer range already admitted vite 8, so the split was a hoisting accident; it is now pinned to a single installation. This is a development-only change — no published bundle differs — but it is what unblocked `@vitejs/plugin-react` 6, whose import of `vite/internal` bound to the packages' vite 8 while running inside Storybook's vite 6 and made every story return HTTP 500.
- **Slack bot Docker build stalled during release** ([#106](https://github.com/aehrc/ecl-lsp/issues/106)): the `docker-image` job hung for 82 minutes on the v1.3.1 release and was cancelled, leaving `ecl-slack-bot` in the registry at 1.3.0 including `:latest`. The archived logs show a single 80-minute gap immediately after `[linux/arm64 build 6/9] RUN npm ci --ignore-scripts` — so it stalled installing the dev toolchain under QEMU emulation and never reached the registry, exonerating the push and its authentication. The build stage compiles TypeScript to plain JavaScript, which is architecture-independent, so it is now pinned to `$BUILDPLATFORM` and runs once natively; only the runtime stage is still built per architecture. Buildx layer caching was added alongside it.
- **Security advisories in the dependency tree**: `fast-uri` (four high, via `@vscode/vsce` → `ajv`) and `qs` (two moderate, via `@slack/bolt` → `express` 5) are pinned to patched releases through `overrides`, and `serialize-javascript` (one high) is resolved by moving `mocha` to 12, whose declared range admits the patched 7.x — mocha's previous `^6.0.2` range had no fixed release. `npm audit` goes from two high and three moderate to none and one. The remaining advisories are all `dompurify`, which `monaco-editor` pins exactly; since monaco is a peer dependency that copy never ships in these packages, and overriding monaco's pin would risk its sanitiser while only silencing local alerts.

### Changed

- **Slack bot runtime dependencies** (`ecl-slack-bot`): `@slack/bolt` 4.7.3 → 5.0.0 and `dotenv` 16.6.1 → 17.4.2. Bolt 5 drops Node 18 (this project already requires 22), removes the deprecated `WorkflowStep` class, replaces axios with native `fetch` for `response_url` calls and removes the `agent`/`clientTls` options — none of which this bot uses. `respond()` now resolves to a `Response` rather than an axios response, and its return value is discarded at all three call sites here. These ship in the published Docker image.
- **Development toolchain**: `vite` 6.4.3 → 8.2.2, `@vitejs/plugin-react` 4.7.0 → 6.1.1, `vite-plugin-dts` 4.5.4 → 5.0.3, `@types/node` 25 → 26, `mocha` 11 → 12, plus Gradle and GitHub Actions group updates. Dropping vite 6 removes `rollup` from the tree entirely, since vite 8 builds on rolldown. None of this reaches a published artifact.
- **Release workflow bounded** ([#107](https://github.com/aehrc/ecl-lsp/pull/107)): every job in `release.yml` now has a `timeout-minutes`, and `github-release` no longer waits on `docker-image` — that job uploads no release asset, and gating the release page on it is what turned the v1.3.1 Docker stall into a stalled release.
- **`.vscode-test/` is ignored**: running the VSCode end-to-end suite downloads roughly 950 MB of VS Code into the working tree. Nothing ignored it, so `npm run lint`, `npm run format:check` and the pre-commit hook all failed afterwards on VS Code's own files.
...
- **FHIR request throttling and `maxConcurrency`** (`ecl-core`, `ecl-lsp-server`, `ecl-editor-core`, `ecl-editor-react`, `ecl-slack-bot`, VS Code) ([#60](https://github.com/aehrc/ecl-lsp/pull/60)): concept lookups and evaluations now go through a concurrency queue with retry and exponential backoff on HTTP 429, preventing rate-limiting on large expressions. The limit is configurable at every consumer surface rather than only in the `FhirTerminologyService` constructor: `maxConcurrency` on the service options and on `RegisterOptions`/`EclEditor` props, a new `EclEditorProvider` React context, `ecl.terminology.maxConcurrency` in VS Code, `ECL_MAX_CONCURRENCY` for the Slack bot, and the Eclipse `initializationOptions`. Defaults to 25. Invalid values are ignored with a warning rather than throwing.
...
- **Build toolchain upgraded**: TypeScript 6, Vitest 4, Storybook 10, jsdom 29, `@types/node` 25 and `eslint-plugin-unicorn` 64, plus an `axios` override bumped to `^1.18.0` for a prototype-pollution advisory. These landed on `main` after 1.1.2 and ship with this release; they affect the build and test tooling, not the published runtime code.
