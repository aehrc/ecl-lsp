...
1. Download the latest `aehrc.ecl-lsp-client-x.x.x.vsix` from Releases
...
| `ecl.formatting.alignTerms`             | `true`  | Vertically align term pipes (no-op)         |
...
## Embedded ECL Support

ECL expressions are recognised inside several host file types. Both **syntax highlighting** (TextMate injection) and **full LSP features** (completions, diagnostics, hover) are active in supported locations.

### FSH files (`.fsh`)

ECL is recognised inside the string literal following `where constraint =`:

```fsh
* codes from system SCT where constraint = "<< 404684003 |Clinical finding|"
```

### FHIR JSON files (`.json`)

ECL is recognised in FHIR ValueSet resources at these locations:

- `compose.include[*].filter[*].value` — when sibling `property` is `"constraint"` or `"expression"`
- `compose.exclude[*].filter[*].value` — same condition
- `expansion.parameter[*].valueString` — when sibling `name` is `"used-ecl"` or `"ecl"`

### FHIR XML files (`.xml`)

Same locations as JSON, in XML ValueSet resources:

- `<filter>` elements where `<property value="constraint"/>` or `<property value="expression"/>`
- `<parameter>` elements where `<name value="used-ecl"/>` or `<name value="ecl"/>`

XML entity encoding (`&lt;` → `<`, `&gt;` → `>`) is handled with precise position mapping.

### Java (`.java`) and TypeScript (`.ts`, `.tsx`) — comment trigger

When `// ecl` or `/* ecl */` appears on the line immediately before a string literal, the string content is treated as ECL:

```java
// ecl
String query = "<< 404684003 |Clinical finding|";
```

```typescript
// ecl
const filter = `<< 404684003 : 363698007 = << 39057004`;
```

Double-quoted, single-quoted, and backtick template literal strings are all supported. The comment trigger must be on the immediately preceding line with no blank lines between.
