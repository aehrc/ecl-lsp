...
let embeddedManager: EmbeddedEclManager;
...
export function activate(context: ExtensionContext) {
  ...
  embeddedManager = new EmbeddedEclManager();
  embeddedManager.registerExtractor('fsh', extractFshFragments);
  embeddedManager.registerExtractor('.fsh', extractFshFragments); // fallback by extension
  embeddedManager.registerExtractor('json', extractFhirJsonFragments);
  embeddedManager.registerExtractor('.json', extractFhirJsonFragments);
  embeddedManager.registerExtractor('xml', extractFhirXmlFragments);
  embeddedManager.registerExtractor('.xml', extractFhirXmlFragments);
  embeddedManager.registerExtractor('java', extractCommentTriggerFragments);
  embeddedManager.registerExtractor('.java', extractCommentTriggerFragments);
  embeddedManager.registerExtractor('typescript', extractCommentTriggerFragments);
  embeddedManager.registerExtractor('.ts', extractCommentTriggerFragments);
  embeddedManager.registerExtractor('typescriptreact', extractCommentTriggerFragments);
  embeddedManager.registerExtractor('.tsx', extractCommentTriggerFragments);
  embeddedManager.activate();
  context.subscriptions.push(embeddedManager);

  embeddedManager.registerProviders([
    { scheme: 'file', language: 'fsh' },
    { scheme: 'file', pattern: '**/*.fsh' },
  ...
  const clientOptions: LanguageClientOptions = {
    ...
    middleware: {
      handleDiagnostics: (uri, diagnostics, next) => {
        if (embeddedManager.handleDiagnostics(uri, diagnostics)) {
          return; // Virtual document diagnostics — handled by embedded manager
        }
...
async function evaluateExpression(client: LanguageClient, startLine?: number, expression?: string): Promise<void> {
  ...
  if (!expression) {
    ...
    const fragment = embeddedManager.getFragmentAtPosition(editor.document.uri, pos);
    if (fragment) {
      expression = fragment.text;
      startLine = fragment.range.start.line;
    } else if (editor.document.languageId === 'ecl') {
      // Fall through to use cursor line on the server
    } else {
      window.showErrorMessage('Place cursor inside an ECL expression to evaluate');
      return;
    }
  }
