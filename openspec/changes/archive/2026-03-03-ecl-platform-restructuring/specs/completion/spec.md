...
The completion provider SHALL analyze the ECL expression context before the cursor position and show only syntactically valid operators at that position. The completion engine SHALL return `CoreCompletionItem` types from ecl-core, which the LSP server adapter maps to LSP `CompletionItem` types.
