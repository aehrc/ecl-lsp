...
The completion provider SHALL analyze the ECL expression context before the cursor position and show only syntactically valid operators at that position. The completion engine SHALL return `CoreCompletionItem` types from ecl-core, which the LSP server adapter maps to LSP `CompletionItem` types. Concept search requests triggered by the completion provider SHALL be debounced so that only the most recent keystroke within a 200 ms window fires a `searchConcepts()` call.
...
#### Scenario: Completion items use CoreCompletionItem type

- **WHEN** the completion engine generates items in ecl-core
- **THEN** items use `CoreCompletionItem` with string literal `kind` values ('keyword', 'operator', 'snippet') instead of LSP `CompletionItemKind` enums

#### Scenario: LSP adapter maps core items to protocol items

- **WHEN** the LSP server receives completion results from ecl-core
- **THEN** it maps `CoreCompletionItem.kind` string literals to `CompletionItemKind` enum values and `CoreInsertTextFormat` to `InsertTextFormat`

#### Scenario: Rapid keystrokes coalesce into a single search request

- **WHEN** the user types multiple characters within 200 ms while the cursor is at a concept search position
- **THEN** only one `searchConcepts()` call is fired (for the final keystroke); intermediate keystrokes are dropped

#### Scenario: Single keystroke with no follow-up fires search after debounce window

- **WHEN** the user types one character and then pauses for more than 200 ms
- **THEN** exactly one `searchConcepts()` call is made after the 200 ms window expires
