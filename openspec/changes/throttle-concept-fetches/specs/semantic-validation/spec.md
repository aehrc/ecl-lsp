...
#### Scenario: Large document dispatches validation through concurrency queue

- **WHEN** a document contains many expressions whose semantic validation would produce more than 5 simultaneous FHIR requests
- **THEN** at most 5 requests execute concurrently; the remainder are queued and dispatched as slots free

#### Scenario: All expressions eventually validated despite queuing

- **WHEN** semantic validation for a large document is queued through the concurrency cap
- **THEN** every expression is validated and its diagnostics are reported once its slot becomes available
