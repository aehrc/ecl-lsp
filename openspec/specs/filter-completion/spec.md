...
Provides context-aware completions inside ECL filter blocks (`{{ D }}`, `{{ C }}`, `{{ M }}`) and history supplements (`{{ + }}`), including filter type openers, filter-specific keywords, and value tokens.
