...
describe('ECL Formatter', () => {
  ...
  describe('Configurable indentation', () => {
    test('should respect indentSize=2 (default)', () => {
      ...
      const result = formatDocument(input, options);
      const lines = result.split('\n');
      // Attribute line should have 2 spaces indent
      const attrLine = lines.find((l) => l.trim().startsWith('{'));
      assert.ok(attrLine?.startsWith('  '), 'Should have 2 space indent');
    });
    ...
    test('should respect indentSize=4', () => {
      ...
      const result = formatDocument(input, options);
      const lines = result.split('\n');
      // Attribute line should have 4 spaces indent
      const attrLine = lines.find((l) => l.trim().startsWith('{'));
      assert.ok(attrLine?.startsWith('    '), 'Should have 4 space indent');
      assert.ok(!attrLine?.startsWith('     '), 'Should not have more than 4 spaces');
    });
    ...
    test('should respect indentStyle="tab"', () => {
      ...
      const result = formatDocument(input, options);
      const lines = result.split('\n');
      // Attribute line should start with tab
      const attrLine = lines.find((l) => l.trim().startsWith('{'));
      assert.ok(attrLine?.startsWith('\t'), 'Should have tab indent');
    });

    test('should handle nested indentation with indentSize=2', () => {
      const input =
        '<< 404684003 | Clinical finding |: 363698007 | Finding site | = << 39057004 | Pulmonary valve structure |, 116676008 | Associated morphology | = << 72651009 | Fracture |';
      const options: FormattingOptions = {
        ...defaultFormattingOptions,
        indentSize: 2,
        breakAfterColon: true,
      ...
      const result = formatDocument(input, options);
      const lines = result.split('\n');
      // Lines after colon should be indented at level 1 (2 spaces)
      assert.ok(lines.length >= 2, 'Should produce multiple lines');
      const attrLine = lines.find((l) => l.trim().startsWith('363698007'));
      assert.ok(attrLine !== undefined, 'Attribute line should exist');
      assert.ok(/^\s{2}\S/.exec(attrLine), 'First level should have 2 spaces');
    });

    test('should handle nested indentation with tabs', () => {
      const input =
        '<< 404684003 | Clinical finding |: 363698007 | Finding site | = << 39057004, 116676008 = << 72651009';
      const options: FormattingOptions = {
        ...defaultFormattingOptions,
        indentStyle: 'tab',
        breakAfterColon: true,
      ...
      const attrLine = lines.find((l) => l.trim().startsWith('363698007'));
      assert.ok(attrLine !== undefined, 'Attribute line should exist');
      assert.ok(attrLine.startsWith('\t'), 'First level should have 1 tab');
    });
  ...
  describe('Configurable line length', () => {
    ...
    test('should break at logical operators when exceeding maxLineLength', () => {
      const input = '< 404684003 OR < 19829001 OR < 234567890';
      const options: FormattingOptions = {
        ...defaultFormattingOptions,
        maxLineLength: 30,
      };
  ...
  describe('Integration: indentation', () => {
    test('indents content inside braces', () => {
      // Use valid ECL with colon before brace group
      const input =
        '< 404684003 |Clinical finding|: { 363698007 |Finding site| = < 39057004 |Pulmonary valve structure|, 116676008 |Associated morphology| = < 72704001 |Fracture| }';
      const result = formatDocument(input, defaultFormattingOptions);
      // AST printer should produce multi-line with braces and indented attributes
      assert.ok(result.includes('363698007'), 'Should have first attribute');
      assert.ok(result.includes('116676008'), 'Should have second attribute');
      assert.ok(result.includes('| Clinical finding |'), 'Should normalize term spacing');
    });

    test('indents multiple attributes in refinement', () => {
      ...
      // Attributes should be indented
      const attrLine = lines.find((l) => l.trim().startsWith('363698007'));
      assert.ok(attrLine !== undefined, 'First attribute present');
      assert.ok(attrLine.startsWith('  '), 'Attribute is indented');
    });
  ...
  describe('Integration: line breaking', () => {
    ...
    test('nested (()) — MINUS indented deeper than AND via paren alignment', () => {
      ...
      // Find the AND line and the MINUS line (AND may be at column 0 for paren-starting expressions)
  ...
  describe('Integration: breakOnOperators option', () => {
    ...
    test('breakOnOperators=true: continuation line with extra operator does not double-indent', () => {
      // Regression: a line that already starts with AND and
    ...
    test('breakOnOperators=true: nested (( keeps parens with content', () => {
      const opts: FormattingOptions = { ...defaultFormattingOptions, breakOnOperators: true };
      const input =
        '((< 404684003 |Clinical finding| OR < 19829001 |Disorder|) AND (< 234567890 |Finding| OR < 345678901 |Morphology|)) MINUS < 456789012 |Exclusion|';
      const result = formatDocument(input, opts);
      const lines = result.split('\n');
      // AST printer keeps parens with their content (no lone '(' lines)
      // First line should start with (( and contain the first concept
      assert.ok(lines[0].startsWith('(('), `first line should start with '((', got: "${lines[0]}"`);
      assert.ok(lines[0].includes('404684003'), 'first line contains first concept');
      // Should break at operators
      assert.ok(
        lines.some((l) => l.trim().startsWith('OR')),
        'OR on its own line',
  ...
  describe('AST printer: depth-aware indentation', () => {
    const opts: FormattingOptions = { ...defaultFormattingOptions, breakOnOperators: true };
  ...
  describe('AST printer: nested paren handling', () => {
    const opts: FormattingOptions = { ...defaultFormattingOptions, breakOnOperators: true };

    test('nested )) kept with content (not split onto separate lines)', () => {
      const input = '((< 404684003 OR < 19829001) AND (< 234567890 OR < 345678901)) MINUS < 456789012';
      const result = formatDocument(input, opts);
      const lines = result.split('\n');
      // AST printer keeps closing parens with their content
      // The last operand group should end with ))
      assert.ok(
        lines.some((l) => l.trim().endsWith('))')),
        `expected a line ending with '))': ${result}`,
      );
      // MINUS should be on its own line
      assert.ok(
        lines.some((l) => l.trim().startsWith('MINUS')),
        'MINUS on its own line',
      );
    ...
    test('closing )) kept inline with content', () => {
      const input = '((< 404684003 OR < 19829001))';
      const result = formatDocument(input, opts);
      const lines = result.split('\n');
      // AST printer keeps )) inline with the last content
      const lastLine = lines.at(-1)!;
      assert.ok(lastLine.trim().endsWith('))'), `last line should end with '))', got: "${lastLine}"`);
    });

    test('triple ((( kept with content', () => {
      const input = '(((< 404684003 OR < 19829001)))';
      const result = formatDocument(input, opts);
      const lines = result.split('\n');
      // AST printer keeps all parens with their content
      assert.ok(lines[0].startsWith('((('), `first line should start with '(((' got: "${lines[0]}"`);
      const lastLine = lines.at(-1)!;
      assert.ok(lastLine.trim().endsWith(')))'), `last line should end with ')))' got: "${lastLine}"`);
    });
  ...
  describe('AST printer: brace groups with breakOnOperators', () => {
    const opts: FormattingOptions = { ...defaultFormattingOptions, breakOnOperators: true };

    test('refined expression with brace group AND another expression', () => {
      const input = '(< 404684003: { 363698007 = < 39057004 }) AND < 19829001';
      const result = formatDocument(input, opts);
      const lines = result.split('\n');
      // Both concepts should be present
      assert.ok(result.includes('363698007'), 'attribute present');
    ...
    test('brace group content is indented inside refinement', () => {
      const input = '< 404684003: { 363698007 = < 39057004 }';
      const resultOpts: FormattingOptions = { ...opts, breakAfterColon: true };
      const result = formatDocument(input, resultOpts);
      const lines = result.split('\n');
      // First line should end with colon
    ...
    test('multiple attributes in brace group are formatted', () => {
      const input = '< 404684003: { 363698007 = < 39057004, 116676008 = < 72704001 }';
      const resultOpts: FormattingOptions = { ...opts, breakAfterColon: true };
      const result = formatDocument(input, resultOpts);
      assert.ok(result.includes('363698007'), 'first attribute present');
  ...
  describe('AST printer: terms with pipes inside parens', () => {
    const opts: FormattingOptions = { ...defaultFormattingOptions, breakOnOperators: true };
  ...
  describe('AST printer: no blank lines guarantee', () => {
    const opts: FormattingOptions = { ...defaultFormattingOptions, breakOnOperators: true };
    ...
    test('expression with refinement and operators has no blank lines', () => {
      const input = '(< 404684003: { 363698007 = < 39057004 }) AND < 19829001';
      const result = formatDocument(input, opts);
      assert.ok(!result.includes('\n\n'), `no blank lines:\n${result}`);
    });
  ...
  describe('AST printer: idempotence', () => {
    const opts: FormattingOptions = { ...defaultFormattingOptions, breakOnOperators: true };
    ...
    test('formatting twice produces same result: refinement + operators', () => {
      const input = '(< 404684003: { 363698007 = < 39057004 }) AND < 19829001';
      const first = formatDocument(input, opts);
      const second = formatDocument(first, opts);
      assert.strictEqual(first, second, 'second pass should be identical to first');
  ...
  describe('AST printer: combined with breakOnRefinementComma', () => {
    ...
    test('operators break + comma breaks work together', () => {
      // Use valid ECL: compound at top level with refinement on first operand
      ...
      assert.ok(result.includes('363698007'), 'first attr present');
      assert.ok(result.includes('116676008'), 'second attr present');
    });
  ...
  describe('breakAfterColon: combined with other break options', () => {
    ...
    test('breakAfterColon + breakOnOperators: colon break + operator break', () => {
      ...
      // Use valid ECL: compound with refined sub-expression in parens
      const input = '(< 404684003: 363698007 = < 39057004) AND < 19829001';
      const result = formatDocument(input, opts);
      const lines = result.split('\n');
